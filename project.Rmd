...
<We load the RNA-seq data set of Kidney renal clear-cell-carcinoma.
```{r}
...
<We take a look to the data of all samples of the sumarized experiment object.
```{r}
dim(colData(se))
colData(se)[1:5, 1:5]
...
<We look through the rowRanges to see the genes information of our data set.
```{r}
rowRanges(se)
```
...
###DGE object
< We create a DGE object with DGEList to hold the dataset to be analysed in a better and more comprehensive way
```{r}
library(edgeR)
