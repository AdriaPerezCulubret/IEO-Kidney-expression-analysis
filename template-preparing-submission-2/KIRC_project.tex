...
\begin{abstract}
Renal Clear Cell Carcinoma is the most common type of Kidney cancer, but very few things are known about his molecular insights. In this study, we perform a differential expression coupled with a function enrichment analysis and a gene set enrichment analysis in order to identify the genes and paths that characterize this type of cancer. We also build a Naive Bayes classifier to decide whether a sample is from a tumour or a normal tissue by the expression data. The results for this study indicates that KIRC tumours suffer a metabolic shift to a downregulation of the oxidative phosphorilation and promotes the fatty-acid synthesis, and it also singles out PD-1 as a key factor for the immune system evasion of the tumour.
...
\caption{Example figure from \url{10.1534/genetics.114.173807}. 
}%
\label{fig:spectrum}
\end{figure}

\subsection*{Hierarchical clustering}
With a Hierarchical clustering approach, we can observe if the strongly DE genes separate the samples in different clusters. We applied this method to the paired samples and we can observe a clear separation between normal and tumour samples in two clusters (Fig. 2A). These results show that these 199 strongly DE genes could be used for building a classification method based on machine learning approach (CTAAA). 

The application of this approach to the whole set of samples results in a more complex set of clusters. As we can see in Fig. 2B, although almost the entire set of normal and tumour samples, cluster together, some tumour samples show a unexpected clustering behaviour. Some of these samples are separated from the huge tumour cluster, while a handful of them are closer to the cluster of normal samples, leading us to determine that these strongly DE genes do not capture the whole variability between tumour and normal samples.


taula amb x mes sobreexpressed, logFC...

3 o 4 genes
GSA: suppl material, els més rellevants...
	Canvi de bias metabolisme: clear cells: s'acumula lipids, son així de clares. Es frena metabolisme (degradació), disminuexi respiració oxidativa, immunnodeficiència, pd1 traget, 

Functional enrichment: cell division, immuno,
